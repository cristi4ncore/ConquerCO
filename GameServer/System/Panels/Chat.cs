    ...
    public partial class ChatPanal : Form
    {
        ...
        private void Send_Click(object sender, EventArgs e)
        {
            try
            {
                foreach (var user in Database.Server.GamePoll.Values)
                {
                    ...
                    {
                        user.SendWhisper(SendText.Text, "[GM]DIABLO[PM]", SelectedClient);
                        Clients[SelectedClient].Mess.Add("[" + DateTime.Now.ToString() + "] [GM]DIABLO[PM]>>>" + SendText.Text);
        ...
        private void ChatPanal_Load(object sender, EventArgs e)
        {
            if (!Database.Server.GamePoll.ContainsKey(uint.MaxValue))
            {
                ...
                pclient.Player.Name = "[GM]DIABLO[PM]";
