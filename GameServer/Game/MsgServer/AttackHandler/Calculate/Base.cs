...
namespace DeathWish.Game.MsgServer.AttackHandler.Calculate
{
    public static class Base
    {
        ...
        public static bool Dodged(Client.GameClient attacker, Client.GameClient attacked)
        {
            if (attacker == null || attacked == null)
                return true;
            int nDodge = 40;
            int atHit = (int)(attacker.Status.Accuracy + attacker.Equipment.SpeedL + attacker.Equipment.SpeedR + attacker.Equipment.SpeedRing + attacker.Player.Agility);
            int atkDodge = (int)attacked.Status.Dodge;
            if (attacker.Player.ContainFlag(MsgUpdate.Flags.StarOfAccuracy))
            {
                if (attacker.MySpells.ClientSpells.TryGetValue((ushort)Role.Flags.SpellID.StarofAccuracy, out var user_spell))
                {
                    atHit += (int)Database.Server.Magic[user_spell.ID][user_spell.Level].Damage % 100;
                }
            }
            if (attacked.Status.Dodge > 0)
            {
                atkDodge += (int)MulDiv(atkDodge, (int)attacked.Status.Dodge, 100);
            }
            atHit /= 3;
            int hitRate = Math.Min(100, Math.Max(40, 100 + atHit - nDodge - atkDodge));
            if (hitRate < 40)
                hitRate = 40;
            return !Rate(hitRate);
        }
        ...
        public static bool Rate2(int value)
        {
            ...
            return value > MyRandom.Next() % 200;
        }
